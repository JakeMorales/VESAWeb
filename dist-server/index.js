...
function precomputeLeaderboardAndStats() {
    ...
    const transformScrimJson = (json) => {
        if (!json || !json.stats || !Array.isArray(json.stats.games))
            return {};
        const result = {};
    ...
    const leaderboard = EloAggregationService.aggregatePlayerElos(scrimFileService, transformScrimJson);
    ...
    const perfMetrics = leaderboard[0]?.performanceMetrics;
    const stats = {
    ...
    scrimObjects.forEach(scrim => {
        if (!scrim || !scrim.stats || !Array.isArray(scrim.stats.games))
            return;
...
app.get('/leaderboard', async (req, res) => {
    ...
    let filteredLeaderboard = cachedLeaderboard.filter(player => player.totalGames >= 18 // Only show players with 18+ games
    );
    const playerNameQuery = typeof req.query.playerName === 'string' ? req.query.playerName.trim().toLowerCase() : '';
    if (playerNameQuery) {
        filteredLeaderboard = filteredLeaderboard.filter(player => typeof player.playerName === 'string' && player.playerName.toLowerCase().includes(playerNameQuery));
    }
...
app.get('/scrims/:filename', (req, res) => {
    try {
...
app.get('/league/seasons', (req, res) => {
    try {
...
app.get('/league/seasons/:season/divisions', (req, res) => {
    try {
...
app.get('/league/seasons/:season/divisions/:division/matches', (req, res) => {
    try {
...
app.get('/league/seasons/:season/divisions/:division/matches/:filename', (req, res) => {
    try {
