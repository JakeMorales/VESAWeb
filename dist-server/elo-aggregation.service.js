 ...
 */
export function printEloCalculationBreakdown(selectedPlayers, selectedGames) {
    selectedPlayers.forEach(player => {
        ...
        selectedGames.forEach((game, idx) => {
            ...
            const eloChange = EloCalculatorService.prototype.calculateEloChangeWithOpponent.call(EloCalculatorService, playerElo, opponentElo, perfScore, gamesPlayed);
...
export class EloAggregationService {
    ...
    static analyzeEloEngagement(playerStats, topN = 50) {
    ...
    static aggregatePlayerElos(scrimFileService, loadScrimTableFromJsonObject) {
        ...
        if (batchTeams.length === 0) {
            ...
            const selectedPlayers = Object.values(playerMapForPrint).slice(0, 3);
            const typedBatchTeams = batchTeams;
        ...
        for (const json of jsons) {
            ...
            Object.entries(matchDay).forEach(([gameNumber, teamGameResults]) => {
                if (!Array.isArray(teamGameResults))
                    return;
                // For each team/game, update player ELOs sequentially
                teamGameResults.forEach((team) => {
                    if (!team.players)
                        return;
                    // Calculate average ELO for the team (for opponent comparison)
                    const teamEloAvg = team.players.reduce((sum, ps) => {
                        const rawName = ps.playerName || ps.name || '';
                        const id = ps.playerId ?? ps.player_id ?? rawName.trim().toLowerCase();
                        if (!playerMap.has(id)) {
                    ...
                    team.players.forEach((ps) => {
                        const rawName = ps.playerName || ps.name || '';
                        const id = ps.playerId ?? ps.player_id ?? rawName.trim().toLowerCase();
                        const playerStats = playerMap.get(id);
                        // Use EloCalculatorService.calculatePerformanceScore with correct per-game damage input
                        const performanceScore = EloCalculatorService.calculatePerformanceScore(ps.placement ?? team.placement ?? 0, ps.kills || 0, ps.assists || 0, ps.damageDealt || 0, ps.revivesGiven || ps.revives || 0);
                        const newElo = EloCalculatorService.prototype.calculateEloChangeWithOpponent(playerStats.elo, teamEloAvg, performanceScore, playerStats.gamesPlayed);
        ...
        playerMap.forEach((val, id) => {
            ...
            const totalDamage = val.stats.reduce((sum, s) => sum + (s.damageDealt || 0), 0);
